...
def convert_roberta_to_htf():
    ...
    htf_config = RoHATConfig.from_pretrained(f'{DATA_DIR}/hat')
    ...
    htf_config.hidden_act = "silu"#roberta_config.hidden_act
    ...
    # load dummy rohat model
    htf_model = RoHATForMaskedLM.from_config(htf_config)

    # copy embeddings
    htf_model.rohat.embeddings.word_embeddings.load_state_dict(roberta_model.roberta.embeddings.word_embeddings.state_dict())
    htf_model.rohat.embeddings.token_type_embeddings.load_state_dict(roberta_model.roberta.embeddings.token_type_embeddings.state_dict())
    htf_model.rohat.embeddings.LayerNorm.load_state_dict(roberta_model.roberta.embeddings.LayerNorm.state_dict())
    ...
    for idx in range(min(NUM_HIDDEN_LAYERS, roberta_config.num_hidden_layers)):
        if htf_model.config.encoder_layout[str(idx)]['sentence_encoder']:
            htf_model.rohat.encoder.layer[idx].sentence_encoder.load_state_dict(roberta_model.roberta.encoder.layer[idx].state_dict())
        if htf_model.config.encoder_layout[str(idx)]['document_encoder']:
            htf_model.rohat.encoder.layer[idx].document_encoder.load_state_dict(roberta_model.roberta.encoder.layer[idx].state_dict())

    # copy lm_head
    htf_model.lm_head.load_state_dict(roberta_model.lm_head.state_dict())

    # save model
    htf_model.save_pretrained(f'{DATA_DIR}/PLMs/rohat-{config.layout}-roberta')

    # save tokenizer
    tokenizer.save_pretrained(f'{DATA_DIR}/PLMs/rohat-{config.layout}-roberta')

    # re-load model
    htf_model = RoHATForMaskedLM.from_pretrained(f'{DATA_DIR}/PLMs/rohat-{config.layout}-roberta')
    htf_tokenizer = RoHATTokenizer.from_pretrained(f'{DATA_DIR}/PLMs/rohat-{config.layout}-roberta')
    print(f'RoBERTa-based RoHAT model with layout {config.layout} is ready to run!')
