...
from models.rohat import RoHATForMaskedLM, RoHATConfig, RoHATTokenizer
...
def convert_bert_to_htf():
    ...
    parser.add_argument('--layout', default='s1', choices=['s1', 's2', 'p1','p1_1', 'p2','p12','p2_1','p2_2', 'e1', 'e2', 'l1', 'l2', 'b1', 'b2', 'f12', 'f8', 'f6'])
    ...
    htf_config = RoHATConfig.from_pretrained(f'{DATA_DIR}/rohat')
    ...
    htf_config.hidden_act = "silu"#bert_config.hidden_act if hasattr(bert_config, 'hidden_act') else bert_config.activation
    ...
    htf_model = RoHATForMaskedLM.from_config(htf_config)

    if config.warmup_strategy != 'none':
        if bert_config.model_type == 'bert':
            htf_model.rohat.embeddings.word_embeddings.load_state_dict(bert_model.bert.embeddings.word_embeddings.state_dict())
            htf_model.rohat.embeddings.token_type_embeddings.load_state_dict(bert_model.bert.embeddings.token_type_embeddings.state_dict())
            htf_model.rohat.embeddings.LayerNorm.load_state_dict(bert_model.bert.embeddings.LayerNorm.state_dict())
        elif bert_config.model_type == 'distilbert':
            htf_model.rohat.embeddings.word_embeddings.load_state_dict(bert_model.distilbert.embeddings.word_embeddings.state_dict())
            htf_model.rohat.embeddings.LayerNorm.load_state_dict(bert_model.distilbert.embeddings.LayerNorm.state_dict())

        if config.warmup_strategy != 'embeds-only':
            for idx in range(NUM_HIDDEN_LAYERS):
                if htf_model.config.encoder_layout[str(idx)]['sentence_encoder']:
                    if bert_config.model_type == 'bert':
                        htf_model.rohat.encoder.layer[idx].sentence_encoder.load_state_dict(bert_model.bert.encoder.layer[idx].state_dict())
                    elif bert_config.model_type == 'distilbert':
                        distil_state = bert_model.distilbert.transformer.layer[idx].state_dict()
                        rohat_state = {}

                        # Attention mappings
                        rohat_state['attention.self.query.weight'] = distil_state['attention.q_lin.weight']
                        rohat_state['attention.self.query.bias'] = distil_state['attention.q_lin.bias']
                        rohat_state['attention.self.key.weight'] = distil_state['attention.k_lin.weight']
                        rohat_state['attention.self.key.bias'] = distil_state['attention.k_lin.bias']
                        rohat_state['attention.self.value.weight'] = distil_state['attention.v_lin.weight']
                        rohat_state['attention.self.value.bias'] = distil_state['attention.v_lin.bias']
                        rohat_state['attention.output.dense.weight'] = distil_state['attention.out_lin.weight']
                        rohat_state['attention.output.dense.bias'] = distil_state['attention.out_lin.bias']
                        
                        # LayerNorm mappings
                        rohat_state['attention.output.LayerNorm.weight'] = distil_state['sa_layer_norm.weight']
                        rohat_state['attention.output.LayerNorm.bias'] = distil_state['sa_layer_norm.bias']
                        rohat_state['output.LayerNorm.weight'] = distil_state['output_layer_norm.weight']
                        rohat_state['output.LayerNorm.bias'] = distil_state['output_layer_norm.bias']
                        
                        # FFN mappings
                        rohat_state['intermediate.dense.weight'] = distil_state['ffn.lin1.weight']
                        rohat_state['intermediate.dense.bias'] = distil_state['ffn.lin1.bias']
                        rohat_state['output.dense.weight'] = distil_state['ffn.lin2.weight']
                        rohat_state['output.dense.bias'] = distil_state['ffn.lin2.bias']
                        
                        htf_model.rohat.encoder.layer[idx].sentence_encoder.load_state_dict(rohat_state)
                
                if htf_model.config.encoder_layout[str(idx)]['document_encoder']:
                    if config.warmup_strategy == 'grouped':
                        if bert_config.model_type == 'bert':
                            htf_model.rohat.encoder.layer[idx].document_encoder.load_state_dict(bert_model.bert.encoder.layer[idx].state_dict())
                        elif bert_config.model_type == 'distilbert':
                            distil_state = bert_model.distilbert.transformer.layer[idx].state_dict()
                            rohat_state = {}
                            ...
                            htf_model.rohat.encoder.layer[idx].document_encoder.load_state_dict(rohat_state)
    ...
    # save model
    htf_model.save_pretrained(f'{DATA_DIR}/PLMs/rohat-{config.layout}-{config.warmup_strategy}')#,safe_serialization=False)

    # save tokenizer
    tokenizer.save_pretrained(f'{DATA_DIR}/PLMs/rohat-{config.layout}-{config.warmup_strategy}')

    # re-load model
    htf_model = RoHATForMaskedLM.from_pretrained(f'{DATA_DIR}/PLMs/rohat-{config.layout}-{config.warmup_strategy}')
    htf_tokenizer = RoHATTokenizer.from_pretrained(f'{DATA_DIR}/PLMs/rohat-{config.layout}-{config.warmup_strategy}')
    print(f'RoHAT model with layout {config.layout} and warm-up strategy {config.warmup_strategy} is ready to run!')
