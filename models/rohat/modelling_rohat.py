...
from transformers.models.roformer.modeling_roformer import RoFormerAttention, RoFormerIntermediate, RoFormerOutput
...
_CONFIG_FOR_DOC = "RoHATConfig"
_TOKENIZER_FOR_DOC = "RoHATTokenizer"

RoHAT_PRETRAINED_MODEL_ARCHIVE_LIST = [
    # "kiddothe2b/hierarchical-transformer-base-4096",
    # "kiddothe2b/adhoc-hierarchical-transformer-base-4096",
    # See all RoHAT models at https://huggingface.co/models?filter=hierarchical-transformer
]
...
@dataclass
class RoHATForBoWPreTrainingOutput(ModelOutput):
    """
    Output type of [`RoHATForPreTraining`].
...
class RoHATForVICRegPreTrainingOutput(ModelOutput):
    """
    Output type of [`RoHATForVICRegPreTraining`].
...
class RoHATForSimCLRPreTrainingOutput(ModelOutput):
    """
    Output type of [`RoHATForSimCLRPreTraining`].
...
class RoHATConfig(PretrainedConfig):
    ...
    to that of the RoHAT `kiddothe2b/hat-base-4096 <https://huggingface.co/kiddothe2b/hat-base-4096>`__ architecture.
...
class RoHATEmbeddings(nn.Module):
    """
    Same as RoFormerEmbeddings with a tiny tweak for positional embeddings indexing.
    """

    # Copied from transformers.models.roformer.modeling_roformer.RoFormerEmbeddings.__init__
...
class RoHATPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, RoHATEncoder):
...
RoHAT_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`RoHATConfig`]): Model configuration class with all the parameters of the
            model. Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""

RoHAT_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `({0})`):
            Indices of input sequence tokens in the vocabulary.

            Indices can be obtained using [`RoHATTokenizer`]. See [`PreTrainedTokenizer.encode`] and
...
class RoHATSentencizer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.activation = nn.Tanh()
...
class RoHATModel(RoHATPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.embeddings = RoHATEmbeddings(config)
        self.encoder = RoHATEncoder(config)
    ...
    @add_start_docstrings_to_model_forward(RoHAT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
...
class RoHATLMHead(nn.Module):
    """RoHAT Head for masked language modeling."""
...
class RoHATSentenceHead(nn.Module):
    """RoHAT Head for masked language modeling."""
...
class RoHATSiameseHead(nn.Module):
    """RoHAT Head for masked language modeling."""
...
class RoHATForMaskedLM(RoHATPreTrainedModel):
    ...
    def get_input_embeddings(self):
        return self.rohat.embeddings.word_embeddings

    def set_input_embeddings(self, value):
        self.rohat.embeddings.word_embeddings = value
...
@add_start_docstrings(
    """
    RoHAT Model with three heads on top as done during the pretraining: a `masked language modeling` head and a `document
    representation prediction ` head and a `masked sentence representation prediction ` head.
    """,
    RoHAT_START_DOCSTRING,
)
class RoHATModelForBoWPreTraining(RoHATPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        if self.config.srp or self.config.srp:
            self.sentencizer = RoHATSentencizer(config)
        if self.config.drp:
            self.pooler = RoHATPooler(config, pooling='max')
    ...
    def forward(
        ...
        return RoHATForBoWPreTrainingOutput(
            loss=total_loss,
...
@add_start_docstrings(
    """
    RoHAT Model with three heads on top as done during the pretraining: a `masked language modeling` head and a `sentence
    projection head` head and a document projection head` head.
    """,
    RoHAT_START_DOCSTRING,
)
class RoHATModelForVICRegPreTraining(RoHATPreTrainedModel):
    def __init__(self, config,
...
class RoHATModelForSimCLRPreTraining(RoHATPreTrainedModel):
    def __init__(self, config,
                 document_regularization=True,
                 sentence_regularization=True):
        super().__init__(config)
...
@add_start_docstrings(
    """
    RoHAT Model transformer with a sequence classification/regression head on top (a linear layer on top of the
...
@add_start_docstrings(
    """
    RoHAT Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
...
class RoHATForMultipleChoice(RoHATPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(RoHAT_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        # checkpoint=_CHECKPOINT_FOR_DOC,
...
@add_start_docstrings(
    """
    RoHAT Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
...
@add_start_docstrings(
    """
    RoHAT Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
